...
function addObjects() {
	...
	// Lenovo komputerlərini əlavə et
	for (var i = 0; i < 200; i++) {
		idCounter++;
		computers.push(
			{
...
console.log("Bütün komputerlərin ümumi sayı = " + computers.length);
...
// 1-ci sifarişin qeydiyyatı
var order1 = {};
order1.id = 1;
order1.note = "2 gün ərzində çatdırılsın";
...
order1.userId = 1;
order1.register = new Date(2020, 05, 23);
order1.totalPrice = calculateOrderTotalPrice(order1);


// 2-ci sifarişin qeydiyyatı
var order2 = {};
order2.id = 2;
order2.note = "4 gün ərzində çatdırılsın və əlavə olaraq 4 ədəd klaviatura olsun";
...
order2.userId = 2;
order2.register = new Date(2020, 08, 11);
order2.totalPrice = calculateOrderTotalPrice(order2);

// 3-cü sifarişin qeydiyyatı
var order3 = {};
order3.id = 3;
order3.note = "12 gün ərzində çatdırılsın və əlavə olaraq 2 ədəd monitor olsun";
...
order3.userId = 3;
order3.register = new Date(2020, 03, 27);
